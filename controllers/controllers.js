...
const getTopPostsByPage = async (req,res) => {
    ...
    try
    {
        postArr = await connection.execute("SELECT * FROM InteractionService.vote RIGHT JOIN InteractionService.interaction ON InteractionService.vote.interactionID = InteractionService.interaction.interactionID LEFT JOIN PostService.post ON InteractionService.interaction.postID = PostService.post.postID WHERE PostService.post.title LIKE ? AND InteractionService.interaction.commentID IS NULL AND InteractionService.interaction.parentID IS NULL GROUP BY InteractionService.interaction.interactionID ORDER BY COUNT(CASE WHEN InteractionService.vote.vote = 1 THEN 1 ELSE NULL END) - COUNT(CASE WHEN InteractionService.vote.vote = 0 THEN 1 ELSE NULL END) DESC LIMIT ? OFFSET ?",[searchQuery,limitPerPage,offset])
    }   
    catch(err)
    {
        await connection.destroy()
...
const getActivePostsByPage = async (req,res) => {
    ...
    try
    {
        postArr = await connection.execute("SELECT * FROM (SELECT interactionID,postID, COUNT(CASE WHEN InteractionService.interaction.commentID IS NOT NULL AND TIMESTAMPDIFF(HOUR,CommentService.`comment`.createdAt,NOW())  <= 24 THEN 1 ELSE NULL END) AS \"activeCommentCount\" FROM InteractionService.interaction LEFT JOIN CommentService.`comment` ON InteractionService.interaction.commentID = CommentService.`comment`.commentID  GROUP BY InteractionService.interaction.postID) AS priority LEFT JOIN PostService.post ON priority.postID = PostService.post.postID WHERE title LIKE ? ORDER BY activeCommentCount DESC LIMIT ? OFFSET ?",[searchQuery,limitPerPage,offset])
    }   
    catch(err)
    {
        await connection.destroy()
